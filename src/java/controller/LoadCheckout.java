...
Gson gson =new Gson();
        ...
        if (httpSession.getAttribute("user") != null) {
            ...
            criteria2.add(Restrictions.eq("userEmail", user));
            criteria2.addOrder(Order.desc("addressId"));
            criteria2.setMaxResults(1);
            Address address = (Address) criteria2.list().get(0);

            //get cities from db
            Criteria criteria3 = session.createCriteria(City.class);
            criteria3.addOrder(Order.asc("cityName"));
            List<City> cityList = criteria3.list();
            
            //get cart items from db
            Criteria criteria4 = session.createCriteria(Cart.class);
             criteria4.add(Restrictions.eq("userEmail", user));
             ...
             cart.setUser(null);
             cart.getProduct().setUserEmail(null);
             }
